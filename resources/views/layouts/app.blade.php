...
<body class="font-sans antialiased text-main" x-data="{ sidebarOpen: false }">
    <div class="min-h-screen flex flex-col lg:flex-row">
        ...
        <div x-show="sidebarOpen" x-transition:enter="transition ease-out duration-300"
            ...
            class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] lg:hidden" style="display: none;"></div>
        ...
        <main class="flex-1 lg:ml-[260px] p-4 lg:p-8 relative">
            ...
            <div
                class="sticky lg:absolute top-0 right-0 left-0 lg:left-0 z-50 
                ...
                <div class="flex items-center gap-3 lg:hidden pt-1">
                    ...
                    <h1 class="text-lg font-bold tracking-tight text-main">AMTECH <span
                            class="text-ev-green italic">EV</span></h1>
                </div>
                ...
                <div class="flex items-center gap-2 lg:gap-3">
                    ...
                    <button @click="sidebarOpen = true"
                        class="lg:hidden p-2.5 bg-glass border border-glass-border rounded-xl text-text-muted hover:text-accent hover:border-accent hover:bg-accent/10 transition-all shadow-sm flex items-center justify-center"
                        title="Menu">
                    ...
                    <button id="theme-toggle"
                        class="p-2.5 lg:p-3 bg-glass border border-glass-border rounded-xl text-text-muted hover:text-accent hover:border-accent hover:bg-accent/10 transition-all shadow-sm group flex items-center justify-center"
                        title="Toggle Theme">
                        <svg id="sun-icon" class="hidden transition-transform group-hover:rotate-45"
                        ...
                        <svg id="moon-icon" class="hidden transition-transform group-hover:-rotate-12"
                            xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="m-0">
                        @csrf
                        <button type="submit"
                            class="p-2.5 lg:p-3 bg-glass border border-glass-border rounded-xl text-text-muted hover:text-red-500 hover:border-red-500 hover:bg-red-500/10 transition-all shadow-sm group flex items-center gap-2"
                            title="Logout">
                            <span
                                class="text-[10px] font-black uppercase tracking-widest hidden sm:inline-block ml-1">Logout</span>
            ...
            <div class="max-w-7xl mx-auto">
                @if (session('success'))
                    <div
                        class="mb-6 p-4 bg-ev-green/10 border border-ev-green/20 text-ev-green rounded-xl text-[10px] font-black uppercase tracking-[0.2em] animate-fade-in flex items-center gap-3">
                ...
                @if ($errors->any())
                    <div
                        class="mb-6 p-4 bg-red-500/10 border border-red-500/20 text-red-500 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] animate-fade-in">
                        <ul class="space-y-2">
