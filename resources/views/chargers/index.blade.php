        ...
        @forelse($chargers as $charger)
            <div class="glass-card overflow-hidden flex flex-col group">
                <div class="relative aspect-square bg-white/5 flex items-center justify-center p-12 overflow-hidden">
                    <img src="{{ $charger->image_url ? (str_starts_with($charger->image_url, 'http') ? $charger->image_url : asset('storage/' . $charger->image_url)) : asset('storage/ev_charger_product_1773856128972.png') }}" alt="{{ $charger->name }}" class="w-full h-full object-contain transition-transform duration-500 group-hover:scale-110">
                ...
                <div class="p-8 flex-1">
                    ...
                    <p class="text-text-muted text-xs leading-relaxed mb-6">{{ Str::limit(strip_tags($charger->description), 100) }}</p>
