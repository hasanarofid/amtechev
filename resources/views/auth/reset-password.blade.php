...
<head>
    ...
    <title>Reset Password – AMTECH EV Specialist</title>
...
<body class="antialiased min-h-screen flex items-center justify-center p-6 hero-bg">
    ...
    <div class="max-w-md w-full my-12">
        <div class="text-center mb-10">
            ...
            <h1 class="text-4xl font-black mb-2 tracking-tight">New Password</h1>
            <p class="text-gray-400">Secure your account with a new password</p>
        </div>
        ...
        <div class="bg-white/5 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/10 shadow-2xl">
            <form method="POST" action="{{ route('password.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $request->email) }}" required readonly class="input-ev opacity-60 cursor-not-allowed">
                    <x-input-error :messages="$errors->get('email')" class="mt-2" />
                </div>

                <!-- Password -->
                <div>
                    <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2 ml-1">New Password</label>
                    <div class="relative" x-data="{ show: false }">
                        <input :type="show ? 'text' : 'password'" name="password" required autofocus class="input-ev pr-10" placeholder="••••••••">
                ...
                <!-- Confirm Password -->
                <div>
                    <label class="block text-xs font-bold uppercase tracking-widest text-gray-400 mb-2 ml-1">Confirm Password</label>
                    <div class="relative" x-data="{ show: false }">
