        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="p-8 rounded-3xl bg-black/40 border border-white/5 hover:border-ev-green/20 transition-all group flex items-start justify-between">
                <div class="flex-1 pr-4">
                    <div class="text-ev-green font-black text-2xl mb-4 tracking-tighter">{{ $qb->name }}</div>
                    <p class="text-gray-400 font-light text-sm">{{ $qb->description }}</p>
                </div>
                <div class="flex-shrink-0">
                    <img src="{{ asset('storage/' . $qb->logo) }}" alt="{{ $qb->name }}" class="h-14 w-auto object-contain transition-all duration-300 transform group-hover:scale-110">
                </div>
            </div>
            @endforeach
        </div>
